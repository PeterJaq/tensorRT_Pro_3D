...
namespace PointPillars{
    ...
    public:
        ...
        virtual bool preprocess(Job& job, const pcl::PointCloud<PointT>& pointcloud) override{
            ...
            Size input_size(input_x_, input_y_, input_z_);
            job.additional.compute(pointcloud.size(), input_size);
            
            preprocess_stream = tensor->get_stream();
            tensor->resize(1, , input_height_, input_width_);

            size_t size_pointcloud = image.cols * image.rows * 3;
        ...
        virtual vector<shared_future<Box3DArray>> commits(const vector<pcl::PointCloud<PointT>>& pointclouds) override{
            return ControllerImpl::commits(pointclouds);
        }

        virtual std::shared_future<Box3DArray> commit(const pcl::PointCloud<PointT> pointcloud) override{
            return ControllerImpl::commit(pointcloud);
        }
